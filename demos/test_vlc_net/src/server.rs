...
impl Server {
    ...
    async fn event_trigger_task(
        ...
        loop {
            match timeout(Duration::from_secs(NET_TIMEOUT), tx.send(value.clone())).await {
                Ok(Ok(_)) => break, // exit
                Ok(Err(_)) | Err(_) => {
                    error!("Network task has been closed, or send publish to network timeout. Retrying in 1 second...");
            ...
            match event {
                ...
                Event::Message(msg) => {
                    ...
                    if let Ok(_permit) = semaphore.try_acquire() {
                        ...
                        warn!("Too many concurrent tasks, dropping message.");
                    }
                }
                _ => {}
            }
