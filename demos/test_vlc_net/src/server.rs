...
const NET_TIMEOUT: u64 = 1; // second
...
impl Server {
    ...
    async fn event_trigger_interval(self: Arc<Self>, tx: mpsc::Sender<OuterMsg>) {
        if self.config.bootnodes.is_none() {
            return;
        }

        if self.config.enable_tx_send {
            let send_interval = Duration::from_micros(self.config.trigger_us);
    ...
    async fn event_trigger_task(
        ...
        if timeout(Duration::from_secs(NET_TIMEOUT), tx.send(value))
            ...
            .is_err()
        {
            error!("Network task has been closed, or send publish to network timeout");
        }
    ...
    async fn event_handler(
        ...
        loop {
            ...
            match event {
                ...
                Event::Message(msg) => {
                    // self.clone().handle_p2p_msg(msg, tx_async_state.clone()).await;
                    let self_clone = self.clone();
                    let tx_state_clone = tx_async_state.clone();
                    let semaphore_clone = semaphore.clone();
                    tokio::spawn(async move {
                        let _permit = semaphore_clone.acquire().await.unwrap();
                        self_clone.handle_p2p_msg(msg, tx_state_clone).await;
                    });
                }
                _ => {}
            }
            ...
            if pubkey.verify(&serialized_event, &received_signature) {
                {
                    ...
                    .is_err()
                    {
                        error!("State task has been closed, or send update state timeout");
                    }
                }
            } else {
        ...
        loop {
            tokio::select! {
                ...
                Some(event) = network.next() => {
                    if timeout(Duration::from_secs(NET_TIMEOUT), rx_net.send(event)).await.is_err(){
                            error!("send to recv handler timeout, event_handler busy.");
                        };
                },
                _ = interval.tick() => {
...
pub async fn run(config: Arc<ServerConfig>) -> anyhow::Result<(), anyhow::Error> {
    let local_key = if config.private_key.is_empty() {
        ...
        let decoded_key = general_purpose::STANDARD.decode(config.private_key.clone())?;
    ...
    if config.bootnodes.is_some() || config.discover != Discover::DHT {
    ...
    let message_sender = tokio::spawn(server_arc.clone().event_trigger_interval(tx_publish));
    ...
    let network_task = tokio::spawn(Server::network_task(swarm, rx_publish, tx_net));
...
mod tests {
    ...
    fn test_stress_ser_unser() {
        ...
        let mut event_net = P2PMsg {
            clock: clock.clone(),
            data: _DEFAULT_PAYLOAD.to_string(),
            pub_key: None,
            signature: None,
        };
        ...
        while start.elapsed() < Duration::from_secs(time_windows_s) {
            ...
            match event {
                ...
                Event::Message(msg) => {
                    server_arc
                        .clone()
                        .handle_p2p_msg(msg, tx_state.clone())
                        .await;
                }
                _ => {}
            }
