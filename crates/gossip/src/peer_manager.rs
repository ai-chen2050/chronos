...
impl TrafficMonitor {
    pub fn new(fraction_to_disconnect: f64) -> Self {
        Self { peer_traffic: HashMap::new(), fraction_to_disconnect }
    }
    ...
    async fn _monitor(&mut self, swarm: &mut Swarm<GossipnetBehaviour>) {
        ...
        loop {
            ...
            let num_to_disconnect = (traffic_list.len() as f64 * self.fraction_to_disconnect) as usize;
    ...
    pub fn top_traffic_peers(&mut self) -> Vec<PeerId> {
        ...
        if ! self.peer_traffic.is_empty() {
            ...
            let top_peers: Vec<PeerId> = traffic_list.iter().take(num_to_disconnect).map(|(&peer_id, _)| peer_id).collect();
            self.peer_traffic.clear();
            result = top_peers;
        }
