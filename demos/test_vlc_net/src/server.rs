            ...
            tokio::select! {
                ...
                _ = check_interval.tick() => {
                    ...
                    if now_proc_memory_bytes > Server::mb_to_bytes(mem.proc_max_mem_mb) {
                        info!("Process memory usage exceeded {:?} MB, current {:?} Bytes, auto disconnect", mem.proc_max_mem_mb, now_proc_memory_bytes);
                        network.disconnect_by_traffic();
                    }
                    if sys_memory_usage_percentage > mem.sys_disconnect_men {
                        info!("Memory usage exceeded {:?}, current usage {:?}, auto disconnect", mem.sys_disconnect_men, sys_memory_usage_percentage);
                        network.disconnect_by_traffic();
                    }
                    if ! mem.is_bootnode && mem.is_sender {
                        if sys_memory_usage_percentage > mem.sys_max_mem_usage {
                            info!("Memory usage exceeded {:?}, current usage {:?}, pausing event trigger task...", mem.sys_max_mem_usage, sys_memory_usage_percentage);
                            ...
                            info!("Memory current usage {:?}", sys_memory_usage_percentage);
                        } else if sys_memory_usage_percentage < mem.sys_min_mem_usage {
                            info!("Memory usage below {:?}, current usage {:?}, proceeding with computation...", mem.sys_min_mem_usage, sys_memory_usage_percentage);
...
pub fn generate_config(
    ...
    let concurrent_verify = num_cpus::get() as u64;
