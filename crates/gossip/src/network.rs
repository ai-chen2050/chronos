...
use crate::peer_manager::TrafficMonitor;
...
impl Network {
    pub fn new(
        ...
        let traffic = TrafficMonitor::new(disconnect_rate);
        Ok(Network {
            local_key,
            multiaddr,
            swarm,
    ...
    pub fn disconnect_by_traffic(&mut self) {
        let peers = self.traffic.top_traffic_peers();
        ...
        for peer_id in &peers {
            ...
            if !is_seed_peer {
                ...
                if ! addr.is_empty() {
                    info!("Disconnecting PeerId: {:?} due to high traffic, Multiaddr: {:?}", peer_id, addr);
        ...
        while let Poll::Ready(maybe_event) = self.swarm.poll_next_unpin(cx) {
            ...
            match event {
                SwarmEvent::Behaviour(GossipnetBehaviourEvent::Gossipsub(
                    ...
                    self.traffic.record_traffic(peer_id, message.data.len() as u64);
